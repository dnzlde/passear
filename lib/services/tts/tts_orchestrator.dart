...
class TtsOrchestrator implements TtsService {
  ...
  final Map<String, List<String>> _persistentCache = {}; // text hash -> list of file paths
  final Map<String, DateTime> _cacheAccessTimes = {}; // text hash -> last access time
  ...
  Future<void> _loadCacheMetadata() async {
    ...
    try {
      final metadataFile = File('${_persistentCacheDir!.path}/$_cacheMetadataFile');
      if (await metadataFile.exists()) {
        ...
        debugPrint('TtsOrchestrator: Loaded ${_persistentCache.length} cached items');
      }
    } catch (e) {
  ...
  Future<void> _saveCacheMetadata() async {
    ...
    try {
      final metadata = {
        ...
        'accessTimes': _cacheAccessTimes.map((key, value) => MapEntry(key, value.toIso8601String())),
      };
  ...
  Future<void> _cleanOldCacheFiles() async {
    ...
    try {
      ...
      debugPrint('TtsOrchestrator: Cache size ($cacheSize bytes) exceeds limit, cleaning...');
      ...
      for (final entry in sortedEntries) {
        if (cacheSize - removedSize <= _maxCacheSizeBytes * 0.8) break; // Keep 20% buffer
        ...
        if (_persistentCache.containsKey(cacheKey)) {
          ...
          for (final filePath in _persistentCache[cacheKey]!) {
            try {
              ...
              debugPrint('TtsOrchestrator: Error deleting cache file $filePath: $e');
            }
          }
  ...
  Future<List<_QueueItem>?> _loadFromPersistentCache(String cacheKey) async {
    if (_persistentCacheDir == null || !_persistentCache.containsKey(cacheKey)) {
      return null;
    }
    ...
    try {
      ...
      debugPrint('TtsOrchestrator: Loaded ${queueItems.length} items from persistent cache');
      return queueItems;
    } catch (e) {
  ...
  Future<void> _saveToPersistentCache(String cacheKey, List<String> tempFilePaths) async {
    ...
    try {
      ...
      for (int i = 0; i < tempFilePaths.length; i++) {
        ...
        if (await tempFile.exists()) {
          ...
          final persistentPath = '${_persistentCacheDir!.path}/${cacheKey}_$i.$extension';
      ...
      if (persistentPaths.isNotEmpty) {
        ...
        debugPrint('TtsOrchestrator: Saved ${persistentPaths.length} files to persistent cache');
  ...
  Future<void> speak(String text) async {
    ...
    // Check caches BEFORE stopping current audio
    ...
    if (persistentItems != null) {
      debugPrint('TtsOrchestrator: Using persistent cached audio - stopping current playback');
      
      // Stop current audio only now that we have cached audio ready
      ...
      _audioQueue.addAll(persistentItems);
    ...
    if (_audioCache.containsKey(cacheKey)) {
      debugPrint('TtsOrchestrator: Using in-memory cached audio - stopping current playback');
    ...
    // Keep current audio playing while we synthesize
    debugPrint('TtsOrchestrator: Audio not cached - synthesizing while current audio continues');
    ...
    try {
      // Create a temporary queue for synthesis
      final tempQueue = <_QueueItem>[];
      final originalQueue = List<_QueueItem>.from(_audioQueue);
      _audioQueue.clear(); // Clear so synthesis populates from scratch
      
      // Synthesize all runs in parallel (for OpenAI) or sequentially (for Piper)
      await _synthesizeAllRuns(runs);
      
      // Save synthesized items
      tempQueue.addAll(_audioQueue);
      
      // Now stop current audio and prepare to play new audio
      debugPrint('TtsOrchestrator: Synthesis complete - stopping current playback');
      _cancellationToken.cancel();
      await stop();
      _cancellationToken.reset();
      ...
      // Activate audio session for new playback
      try {
        if (_audioSession != null) {
          await _audioSession!.setActive(true);
          debugPrint('Audio session activated for speaking');
